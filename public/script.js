...
function buildTable(output,loanAmount,monthlyRate,totalNumberOfPayments,payment) {
	...
	{ 
		...
		amortizationTable += "<td style='border: 1px solid black; overflow-x:scroll'> $" + parseFloat(loanAmount).toFixed(2) + "</td>";
		
		//interest payment for this row
		interest = loanAmount * monthlyRate;
		amortizationTable += "<td style='border: 1px solid black; overflow-x:scroll'> $" + parseFloat(interest).toFixed(2) + "</td>";
		
		//principal payment for this row
		monthlyPrincipal = payment - interest;
		amortizationTable += "<td style='border: 1px solid black; overflow-x:scroll'> $" + parseFloat(monthlyPrincipal).toFixed(2) + "</td>";
